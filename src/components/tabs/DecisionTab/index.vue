  ...
  for (const section of currentSections.value) {
    ...
    if (section.type === "sub-checklists") {
      for (const group of section.groups) {
        payload.checklist[group.id] = activeState.checklist[group.id] ?? false;
...
const handleAction = async (actionKey) => {
  ...
  if (actionKey === "send") {
    try {
      const response = await ideaService.submitDecision(
        tabData.value.ideaId,
        payload,
      );
      if (response.success) {
    ...
    try {
      const response = await ideaService.saveDecisionDraft(
        tabData.value.ideaId,
        payload,
      );
      if (response.success) {
      ...
      <BaseBox type="glass" class="p-4 sm:p-6 mb-6">
        ...
        >
          ...
          <div v-else-if="section.type === 'board-info'" class="mb-6">
            <h4
              v-if="section.title"
              class="text-theme-text text-xs md:text-sm font-medium mb-3"
            >
              {{ section.title }}
            </h4>
            <BaseBox
    ...
    >
      ...
      <div class="flex flex-col sm:flex-row items-center gap-3 mt-1">
        <BaseButton
          ...
          class="w-full sm:w-auto min-w-[180px] rounded-xl rtl:bg-linear-to-r ltr:bg-linear-to-l from-[#05D989] to-[#018AAF] text-theme-text font-bold"
        >
