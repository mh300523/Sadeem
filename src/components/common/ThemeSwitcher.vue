...
<template>
  <button
    @click="toggleTheme"
    class="relative flex items-center w-16 h-10 rounded-full p-1 shadow-md transition-colors duration-300 cursor-pointer overflow-hidden"
  ...
  >
    <div
      ...
      :class="isDark ? 'translate-x-0 bg-[#060e1d]' : '-translate-x-6 bg-white'"
    >
      <!-- Moon (dark mode active) -->
      <svg
        v-if="isDark"
        xmlns="http://www.w3.org/2000/svg"
        class="h-4 w-4 text-[#a5b4fc]"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
